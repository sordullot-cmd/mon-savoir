...
_ICI = os.path.dirname(os.path.abspath(__file__))          # .claude/skills/_lib
sys.path.insert(0, _ICI)
sys.path.insert(0, os.path.join(os.path.dirname(_ICI), "inspi"))
import shoot    # capteur pleine hauteur (CDP), vit avec le skill inspi
import palette  # read_png (décodeur PNG pur Python), voisin dans _lib
...
def repartir(n: int, cols: int) -> list[int]:
    """Découpe n visuels en lignes d'au plus `cols`, sans ligne orpheline : la
    dernière ligne remplit la largeur comme les autres, jamais un seul visuel
    laissé avec du vide à droite (même règle que la mise en page du site)."""
    if n <= cols:
        return [n]
    lignes = -(-n // cols)
    while lignes > 1 and n // lignes < 2:
        lignes -= 1
    base, reste = divmod(n, lignes)
    return [base + (1 if i < reste else 0) for i in range(lignes)]


def build_html(fichiers, cols, tile, bg, titre, legende) -> str:
    page_bg, fg, tile_css = FONDS[bg]
    lignes_html, i = [], 0
    for taille in repartir(len(fichiers), cols):
        cases = []
        for f in fichiers[i:i + taille]:
            i += 1
        ...
        # une ligne = une grille de `taille` cellules égales, sur toute la largeur
        lignes_html.append(
            f'<div class="ligne" style="grid-template-columns:repeat({taille},1fr)">{"".join(cases)}</div>')
...
<style>
  *{{box-sizing:border-box}}
  ...
  .grille{{display:flex;flex-direction:column;gap:18px}}
  .ligne{{display:grid;gap:18px}}
  figure{{margin:0}}
...
{entete}<div class="grille">{''.join(lignes_html)}</div>"""
