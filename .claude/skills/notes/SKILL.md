...
- **Alimenter le hub de la famille** — **obligatoire à chaque passage**, demandé le 10 septembre 2026 : « j'aimerais que toute nouvelle note sur le trading soit aussi integrée dans la page synthese trading, comme ça la page est continuellement abreuvée ».
  - **`#trading` → [[synthese trading]]**. Une note trading nouvelle, ou une ligne nouvelle dans une note trading existante, doit apparaître dans la synthèse : une ligne courte dans la bonne rubrique, finissant par `→ [[note d'origine]]`. Une note trading absente du hub est un passage raté.
  - **la vérification se fait note par note, pas de mémoire** : relire chaque note `#trading` et cocher que chacune de ses sections a sa ligne dans le hub. C'est ce qui a rattrapé, le 10 septembre, les lignes de `powell` (amd, obs, fvg cloturé, cisd) et de `sdtv` (time frame, london, hod / lod, accumuler les stdv, pas de buy limit) qui manquaient depuis des semaines.
  - **le hub résume, il ne recopie pas.** Une ligne par élément, la version courte, le détail reste dans la note d'origine. `verifie.py --synthese=<nom>` refuse au-delà de 25 % du dossier.
  - **ce qui cloche remonte aussi** : la section `a trancher :` du hub rassemble ses questions ouvertes et les chiffres douteux, avec la note d'origine entre parenthèses.
  - même logique pour les autres familles quand elles auront leur hub. Aujourd'hui seul `#trading` en a un.
